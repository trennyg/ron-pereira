          ...
          {submitted ? (
            <div className="border border-[var(--gold-border)] p-10 text-center">
              ...
              <p className="font-[var(--font-cormorant)] text-[var(--cream-dim)] text-[1rem] leading-[1.8]">
                Ron will be in touch within 24 hours.
              </p>
            </div>
          ) : (
            <form className="flex flex-col gap-4" onSubmit={e => { e.preventDefault(); setSubmitted(true) }}>
              ...
              <div className="grid grid-cols-2 gap-4 max-sm:grid-cols-1">
                <div className="flex flex-col gap-[0.4rem]">
                  ...
                  <select defaultValue={preSelected??''} className="bg-[rgba(201,168,76,0.03)] border border-[var(--gold-border)] text-[var(--cream)] px-4 py-[0.9rem] font-[var(--font-cormorant)] text-[1rem] outline-none focus:border-[var(--gold)] transition-colors">
                    <option value="">Select Service</option>
                    {SERVICES.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
                  </select>
                </div>
                <Field label="Event Date" type="date" placeholder="" />
              </div>
              {preAct && (
                <div className="flex flex-col gap-[0.4rem]">
