...
export default function Nav() {
  ...
  return (
    <>
      <nav className="fixed top-0 left-0 right-0 z-[700] flex justify-between items-center px-14 py-5 transition-all duration-500 max-md:px-5 max-md:py-4"
        ...
        {/* Logo — R white, P gold */}
        <Link href="/" className="font-[var(--font-cinzel)] font-black flex-shrink-0" style={{zIndex:810, fontSize:'0.88rem', letterSpacing:'0.12em'}}>
          <span style={{color:'var(--cream)'}}>R</span><span className="gold-shimmer">P</span>
        </Link>

        {/* Desktop links */}
        <ul className="hidden md:flex gap-10 list-none">
          {[{href:'/#collabs',label:'Collabs'},{href:'/#booking',label:'Book'}].map(l => (
            <li key={l.href}>
              <Link href={l.href} className="font-[var(--font-mono)] text-[0.54rem] tracking-[0.35em] uppercase text-[var(--cream-dim)] hover:text-[var(--gold)] transition-colors duration-300">{l.label}</Link>
            </li>
          ))}
          ...
          <li className="relative group">
            ...
            {/* Dropdown */}
            <div className="absolute right-0 w-52 opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto transition-all duration-300 translate-y-1 group-hover:translate-y-0"
        ...
        {/* Hamburger */}
        <button className="md:hidden flex flex-col gap-[5px] w-7 p-1 cursor-pointer" style={{zIndex:810}}
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div initial={{x:'100%'}} animate={{x:0}} exit={{x:'100%'}}
            ...
            {/* Regular links */}
            {[{href:'/#collabs',label:'Collabs'},{href:'/#booking',label:'Book'}].map((l,i) => (
              <motion.div key={l.href} initial={{opacity:0,y:24}} animate={{opacity:1,y:0}} transition={{delay:i*0.06}}>
